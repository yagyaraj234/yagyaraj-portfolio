...
So even if your image is compressed, if it's loaded through JavaScript or blocked behind heavy scripts, LCP will still be bad.
...
This tells the browser to load that image first. That one change alone can drop LCP by 300–800ms.
...
Sending a 2000px image to a mobile phone is just wasted bandwidth.
...
Without this, a phone downloads the same oversized image meant for a 27-inch monitor. That's bad for performance and bad for users on slow connections.
...
Only use this when the design actually needs different crops. Don't add it everywhere it's extra work for minimal gain in most cases.
...
2. Loading images after a large JS bundle finishes
3. Sending desktop-size images to all devices
4. No width/height causing layout shift
5. Ignoring pixel density
6. Adding LQIP to every single image
7. Using `background-image` for the main content image
