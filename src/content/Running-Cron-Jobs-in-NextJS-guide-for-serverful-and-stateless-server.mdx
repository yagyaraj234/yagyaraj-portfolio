...
Before diving into how to setup cron jobs. let me explain you what are **cron-jobs** are how they are useful in building in systems.
A cron job is some kind of <Annotation type="underline">**schedular**</Annotation> which will run automatically at specific interval like hourly, daily, weekly, or at custom schedules.
...
In simple terms any repeative task we want to do in specific interval we run setup cron job for it.
...
![logs from vercel schedular](/post/running-cron-jobs-in-nextjs-guide-for-serverful-and-stateless-server/api-log.png)
